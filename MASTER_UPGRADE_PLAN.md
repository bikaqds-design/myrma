...
| A6 | 🟠 Medium → 🟡 **Partly closed 2026-08-07** | **Testing gate stated but not honored** — Sprints 3/5/6/7 marked COMPLETE with QA boxes unchecked; 277 unit tests cover only `src/lib/` pure fns — zero integration/RLS/RPC coverage. | Definition of Done; 52-item checklist unrun | `BUILT` vs `VERIFIED` split; integration tier + deferred 52-item checklist run as Sprint 8 gate. **Progress:** the 52-item checklist, Warehouse R1 and the Purchase Module have all now been run manually and are marked VERIFIED with evidence — 20 defects found and fixed across the three runs. **Still open:** (a) the `db-tests` RPC/RLS tier has *never* executed — the harness added for this very finding was itself unverified, and a "VERIFIED" marker was written on the strength of it (corrected in the Track A summary); (b) Sprints 2.5 and 3 remain un-QA'd; (c) there is still no automated integration tier — all of the above is manual evidence. |
