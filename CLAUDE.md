...
Delivered units are not stock on hand (2026-09-17, Warehouse R1 re-run):

- `20260874_delivered_units_not_on_hand.sql` — a unit delivered on a sales order keeps `status = 'company_stock'` with `reservation_status = 'delivered'` (`restore_units` and `link_serial_to_rma_ticket` rely on it), so **every "on the shelf" count must leave delivered units out**: `v_product_stock_summary` Main / branches / Physical Total, `v_warehouse_unit_counts`, `archive_warehouse`'s live-stock check, and the Stock Breakdown modal. Available, Reserved and the Delivered figure are unchanged. Pinned by `src/test/deliveredUnitsNotOnHand.test.js` and `supabase/tests/inventory_list_views.sql` (10/10 on production, rolled back).
