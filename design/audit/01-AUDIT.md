...
| 🟡 UX-GLOBAL-007 (primitive built; 1 of 72 migrated) | All | No shared Table. 72 raw `<table>` across 50 files | Consistency | High | L | Build `Table` with sort/filter/pagination/bulk-select; migrate per module | `src/components/ui/Table/` (empty dir) |
