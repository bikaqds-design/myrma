...
| 🟡 UX-GLOBAL-007 (primitive built + footer support; 4 of ~64 migrated) | All | No shared Table. 72 raw `<table>` across 50 files | Consistency | High | L | Build `Table` with sort/filter/pagination/bulk-select; migrate per module | `src/components/ui/Table/` (empty dir) |
