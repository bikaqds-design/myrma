...
- **Track A — Inventory & Warehouse Management** *(Sprint 8, ✅ BUILT 2026-07-01, UI **VERIFIED** 2026-08-07; RPC layer **still unproven** — see the correction at the end of this entry)*: dual stock-tracking model (serialized + bulk `warehouse_stock`); rebuilt Inventory page (funnel-aware Overview, Stock Breakdown modal, dual-mode Receive Stock, Stock Movements tab) alongside the 5 preserved RMA-ticket-status tabs; warehouse types/manager/archive; atomic transfer/adjust; bulk action bar. 6 schema migrations (`20260740`–`20260745`) applied and confirmed. **Manual UI click-through QA + the deferred Sprint 6/7 52-item funnel checklist are ✅ DONE (2026-08-07)** — all 52 rows run, all 52 pass; evidence in [`WAREHOUSE_R1_TEST_CHECKLIST.md`](WAREHOUSE_R1_TEST_CHECKLIST.md). That run found and fixed 5 defects (silent ticket-activity write failure, unpostable standalone invoices, no-op invoice rejection, English mobile headers, per-doctype approval labels).

> ⚠️ **Correction 2026-08-07 — this entry previously claimed "RPC layer VERIFIED 2026-07-02" and that
> `inventory_hardening.sql` + `inventory_hardening2.sql` "all pass live" via the `db-tests` CI harness.
> That was never true.** The `db-tests` job has failed on every single run since it was added in `9cc2be4`
> on 2026-07-02 and has never once been green, so those 26 checks have never executed. Cause: `supabase
> start` rebuilds the schema from migrations alone, but 13 core tables (`products`, `customers`,
> `rma_tickets`, `inventory_units`, `warehouses`, …) are created by no migration — they came from Base44 —
> so the job dies before the first assertion. The job is now parked behind `if: false` with the full
> explanation in `.github/workflows/ci.yml`; it needs a baseline migration to work.
>
> **The RPC layer is therefore BUILT, not VERIFIED.** Much of its behaviour was exercised by hand during
> the 2026-08 QA runs (reservation conservation, over-reservation rejection, duplicate-serial rejection,
> transfer/adjust system-warehouse guards), but that is manual evidence, not the automated proof this
> entry asserted. This is exactly the failure mode audit item A6 describes, repeating itself.
- **Track A — Purchase Module** *(Sprint 9, ✅ BUILT 2026-07-01; **"RPC layer VERIFIED 2026-07-02" retracted 2026-08-07** — the `db-tests` harness that claim rested on has never run, see the Sprint 8 correction above. Superseded by the 9R redesign, which **is** QA-verified)*: Vendor → Proforma Invoice → Purchase Order → Vendor Invoice → atomic dual-mode receipt; `/purchasing` page; supports partial (multi-shipment) receipt; full procurement audit trail via `vendor_invoice_id` FK + `stock_moves`. 5 migrations (`20260746`–`20260750`) applied and confirmed. **`receive_vendor_invoice` now covered by `db-tests`**: bulk full receipt + `vi_code` assignment, serialized partial-then-complete receipt (same code reused), duplicate-serial-across-invoices rejection — all pass live. Manual UI click-through (Stock Movements tab origin display, Arabic/dark-mode) still pending.
...
- **Track A — Purchase Module Redesign** *(2026-07-05, ✅ BUILT · ✅ **QA VERIFIED 2026-08-07**)*: Brands ARE the vendors (standalone `vendors` table + Proforma Invoices removed, prior purchasing data wiped per user approval); funnel is **Purchase Order → Vendor Invoice** with an approval-pool step; PO PDF export; full Vendor Payments (AP) ledger. Migrations `20260756`–`20260763`. **Click-through QA ✅ done 2026-08-07** — vendor → PO → approve → PDF → VI → approve → partial receipt → full receipt → AP payment → aging → void, all verified; evidence in [`WAREHOUSE_R1_TEST_CHECKLIST.md`](WAREHOUSE_R1_TEST_CHECKLIST.md). Note the receive path had **never** been exercised before that run: the module's only vendor invoice sat at Approved with code `Pending`, so no `VI-` number had ever been issued and no stock had ever entered through purchasing. Four defects found and fixed, including purchase receipts being allowed into protected system warehouses (`Scrap`, `RMA-*`).
- **Track A — Warehouse Module Redesign R1** *(2026-07-09, ✅ BUILT · ✅ **QA VERIFIED 2026-08-07**)*: RMA ticket stages become **real inventory locations** — 8 protected system warehouses (`RMA-*`/`REPLACEMENT`/`CREDIT-NOTE`/`SCRAP`), units auto-move between them on every ticket save (`move_rma_units`, idempotent), and `promote_rma_unit` is the previously-missing active_rma→sellable path (`resolve_units()` had zero callers). Inventory Overview rebuilt into a **Warehouse Dashboard** (Available/Reserved/Physical Total/Main/Branches/RMA + drill-down drawers); the 5 old RMA-stage tabs and `ProductStatusTab.jsx` removed; stale `?tab=` deep links fall back to Overview. Migrations `20260764`–`20260767` + `inventory_hardening3.sql` (12 checks). Gate-green (287/287 tests, 0 lint errors, build ✓). **Manual click-through QA ✅ COMPLETE (2026-08-07)** — [`WAREHOUSE_R1_TEST_CHECKLIST.md`](WAREHOUSE_R1_TEST_CHECKLIST.md) finished at 70 ✅ / 2 ⏸️ (parked by decision, not by failure) — **and one of those two, reserve/deliver on an SO/Invoice, was run and passed on 2026-08-16, turning up BUG #44 (an invoice can post and bill while delivering zero units; fix prepared as migration `20260775`)**. The run found and fixed 11 defects, including a class of bug where the "manual operations may not target a system warehouse" rule had been written inline four separate times and was wrong in all four — now one shared `destinationWarehouses()` helper, applied at five call sites. R2 deferred: server-side sellable-location guard in `reserve_units`, document-driven onward moves (CN/replacement), and consolidating the deprecated unledgered `transferUnits` path.
- **Track A — CRM Leads/Pipeline round** *(2026-08-04/05, ✅ BUILT · ✅ **QA VERIFIED 2026-08-09**)*: closes all 8 findings from the 2026-07-05 manual test (`MyCRM Manual Test 05-07-2026.xlsx` — Leads + Pipeline only; the funnel past QT→SO was never exercised) plus 3 gaps found while working. Leads: All/Active/Converted/Disqualified tabs, Kanban scoped to the All tab, reopen-disqualified, inline full-name edit fixed (the field rendered `null` while editing, so it vanished until refresh). Deals: **edit modal restored** — it was fully built but nothing called `handleOpenEdit`, so its only entry point had been lost — with customer, contact and pipeline now editable (`deals.movePipeline()` for the stage remap). **A deal now holds many quotations**, each independent, and **deal value is forecast while open / actual once closed** (`src/lib/dealValue.ts`, 18 Vitest cases). Quotations lock at `accepted`; Cancel replaced by Archive wired to the Sales Documents Archive tab. Gate-green (305/305 tests, 0 lint errors, build ✓). **`lint:ci` was failing on HEAD before this round** — 3 pre-existing unused vars vs. `--max-warnings 0`, so the CI job would have gone red on the next push regardless. **Manual click-through QA ✅ COMPLETE 2026-08-09** — [`CRM_QA_CHECKLIST.md`](CRM_QA_CHECKLIST.md) rows 41-48. All eight items verified: Leads Kanban scoped to All (34 cards = the All count), the inline full-name `null` fix, the restored deal edit modal, editable customer/pipeline with `movePipeline()` remapping the stage on a B2B→B2C move, many-quotations-per-deal with independent statuses, **forecast-vs-actual deal value proven both ways** (4,250 open → 1,500 on win, the unconverted quotation correctly dropping out), quotation locking at `accepted`, and Cancel→Archive. One defect found and fixed (**#23**, a doubled `+` on the add-line control). Two things noted rather than closed: the **Contact** field never appeared because the test customer has no contacts on file, and `canMarkDealWon()` / the free-form-line conversion guard both look like bugs until you read the code — documented in the checklist so the next reader skips the detour.
...
## Sprint 2.5 — Professional Lead Experience (Odoo/Zoho-grade) ✅ BUILT · ✅ **QA VERIFIED 2026-08-08**

> **QA run 2026-08-08** — [`CRM_QA_CHECKLIST.md`](CRM_QA_CHECKLIST.md) rows 1-17. Two defects
> found and fixed: **#21** bulk status changes wrote nothing to the activity log (the single-lead
> paths were fine, the bulk path bypassed `updateStatus()` entirely — a lead was found sitting at
> "New" whose newest history entry read *"changed to Disqualified"*), and **#22** the Disqualify
> confirmation's button said **Delete** even though nothing is deleted.
>
> Note this section previously ended "Sprint 2.5 is fully closed… built and verified" while the
> heading still said "QA pending". Everything was re-verified from scratch rather than trusting
> either claim — which is how #21 surfaced.
...
## Sprint 3 — Pipeline Kanban ✅ BUILT · ✅ **QA VERIFIED 2026-08-08**

> **QA run 2026-08-08** — [`CRM_QA_CHECKLIST.md`](CRM_QA_CHECKLIST.md) rows 18-40. **No functional
> defects found**: drag-and-drop moves and persists, drop-on-Won forces probability to 100,
> drop-on-Lost blocks until a reason is given, Won/Lost cards are correctly undraggable (you leave
> a closed deal via Reopen, which offers only open stages), all five views render, inline editing
> persists, and the Comment Panel ↔ Deal Log sync works.
>
> Partly closed 2026-08-16: attempting row 40 found **BUG #43** — at 375px the board crushed all
> seven columns to 34px each and was unusable, fixed and verified at both widths. The **drag
> gesture** is still unverified: `@hello-pangea/dnd`'s touch sensor needs press → hold 120ms →
> move, which neither real nor synthetic input in this harness produces (the same input fails
> identically on the untouched RMA Tickets board, so it is a harness limit, not a finding). Needs a
> real device — but there is now a usable board to test on.
>
> Doc drift found: deal codes are **`OPP-`**, not the `QT-` recorded in Round 4 (a later rename
> was never written down), and the third deal tab is **Quotation**, not "Product Lines".
...
- [x] Full click-through (both paths): Deal → Quotation tab → Convert to SO → Convert to Invoice → PDF; standalone from Invoicing tab for each document type — ✅ 2026-08-07, both paths. The standalone path was in fact **broken** and is the reason this box could not have been ticked before: a standalone invoice raised no approval activity, so it sat in draft with no Post button and no pool row. Fixed.
- [ ] Free-form product promotion: create a quotation with a non-DB product name → convert to SO → confirm the "new product" modal → verify the product now exists in the catalog — **still pending**, not covered by the 52-item run
- [~] Inventory cycle: SO confirm reserves units → Invoice post decrements → Credit Note (rma_return) restores; verify `stock_moves` ledger rows for each step — **partly done 2026-08-07.** Reserve-on-approve and release-on-cancel verified against live counts (15→13→15, reserved 0→2→0, physical unchanged), and `stock_moves` rows confirmed for the purchasing receive path. **Not verified:** that an `rma_return` credit note restores stock — the credit notes exercised were `discount` type, so that leg is untested.
- [ ] Invoice code gapless check: delete two draft invoices, post a third → verify INV- sequence has no gap — **still pending**, not covered by the 52-item run
- [x] Credit note applied: issue a CN against an invoice, apply it → verify `remaining_balance` reaches 0 and status flips to `applied` — ✅ 2026-08-07. `CN-2026-00001` (20.00) took `INV-2026-00017` from 60.00 → 40.00 remaining and stayed `partial`; `CN-2026-00002` (40.00) took it to 0.00 and flipped it to `paid`. Both credit notes land on status `applied` when linked to an invoice (an unlinked one issues as `issued`).
...
| A6 | 🟠 Medium → 🟡 **Partly closed 2026-08-09** | **Testing gate stated but not honored** — Sprints 3/5/6/7 marked COMPLETE with QA boxes unchecked; 277 unit tests cover only `src/lib/` pure fns — zero integration/RLS/RPC coverage. | Definition of Done; 52-item checklist unrun | `BUILT` vs `VERIFIED` split; integration tier + deferred 52-item checklist run as Sprint 8 gate. **Progress: no sprint is marked "QA pending" any more.** Every one has been run manually and is VERIFIED with evidence — the 52-item funnel checklist, Warehouse R1 and the Purchase Module (`WAREHOUSE_R1_TEST_CHECKLIST.md`), plus Sprints 2.5 + 3 and the CRM Leads/Pipeline round (`CRM_QA_CHECKLIST.md`). 23 defects found and fixed across the five runs. **(b) CLOSED 2026-08-16 — an automated integration tier now exists and runs on every push.** `npm run test:integration` (`tests/integration/`, 25 assertions, `vitest.integration.config.js`, CI job `DB · Integration`). It targets the **hosted** Supabase project instead of a container, which sidesteps both blockers at once: no Docker, and no baseline migration either, because the target already has the schema Base44 created. Covers RLS refusal for anonymous callers across 8 protected tables, authorization on 4 `SECURITY DEFINER` RPCs, and schema-drift checks on every column the app selects. A connectivity suite runs first and fails loudly if the target is not a live PostgREST — without it a wrong URL turns every refusal-based assertion green, which was verified by pointing it at a bad host and watching 24 tests pass while only the guard failed. Skips cleanly with no credentials, so forks stay green. <br><br>**Still open:** (a) the `db-tests` RPC/RLS tier has *never* executed — the harness added for this very finding was itself unverified, and a "VERIFIED" marker was written on the strength of it (corrected in the Track A summary). It is now **superseded rather than pending**: Docker is ruled out, so the job should be deleted once its `supabase/tests/*.sql` assertions are ported to the integration tier. (c) The integration tier reads and asserts refusals; it does not exercise the **write** paths those RPCs guard, because doing so against the live project would create real records. `describeWrite` is wired and gated behind `SUPABASE_TEST_ALLOW_WRITES`, waiting on a second hosted project to point at. |
...
**Sprint 8 (Phases 8a/8b/8c) is fully BUILT, and its UI is now VERIFIED (2026-08-07).** The manual
click-through QA (Overview, Stock Breakdown, Receive Stock, Stock Movements, Warehouse archive, bulk
actions) and the deferred Sprint 6/7 52-item funnel checklist have both been run —
[`WAREHOUSE_R1_TEST_CHECKLIST.md`](WAREHOUSE_R1_TEST_CHECKLIST.md).

Two caveats before marking the sprint COMPLETE outright:

1. **The RPC layer is not automatically verified.** The `db-tests` harness this sprint relied on has
   never executed successfully (see the correction in the Track A summary). Its 26 checks remain
   unrun. Much of the same ground was covered manually, but that is a weaker guarantee.
2. **The bulk half of the dual-tracking model was unreachable until 2026-08-07.** `stock_tracking_mode`
   was read by 8 components and written by none, so every product sat on `serialized` and the bulk
   `warehouse_stock` paths — including this sprint's headline dual-mode Receive Stock — were dead code
   in practice. Now exposed in the product form and guarded by `20260772`; a bulk product has since
   been created and received end to end.
...
- [x] **Deferred Sprint 6/7 52-item funnel checklist runs and passes HERE** — ✅ done 2026-08-07, all 52 rows run and passing. One row (46) turned out to be the *checklist* being stale rather than the code: it expected voiding an applied payment to be blocked, but `20260754` deliberately made it reverse-then-void. Row reworded, code left alone. Evidence: [`WAREHOUSE_R1_TEST_CHECKLIST.md`](WAREHOUSE_R1_TEST_CHECKLIST.md)
...
## Sprint H — Security & DB-Test Hardening ✅ COMPLETE 2026-07-02 · ⚠️ see the DB-test caveat below

> ⚠️ **Caveat added 2026-08-07.** The security half of this sprint stands. The **DB-test half does
> not**: the `db-tests` CI harness introduced here (commit `9cc2be4`, 2026-07-02) has failed on every
> run since and has never been green, so none of the SQL assertion suites it was built to run —
> `audit_hardening.sql`, `inventory_hardening.sql` ×3, `vendor_payments.sql` — has ever actually
> executed. The suites are written and committed; they have simply never been proven.
>
> Cause: the harness rebuilds the schema by applying migrations to an empty Postgres, but 13 core
> tables came from Base44 and are created by no migration, so `supabase start` fails before the first
> assertion. The job is parked behind `if: false` with the full explanation in
> `.github/workflows/ci.yml`; a baseline migration unblocks it. `npm run test:db` is broken for the
> same reason.
>
> Where findings in this sprint say "verified live", that means verified by hand or in the Supabase
> SQL editor at the time — **not** by the automated harness.
