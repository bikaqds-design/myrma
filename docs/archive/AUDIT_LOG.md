...
|----|------|-------------|---------|-----|
| **C-1** | CI/Tests | [vite.config.js:81-97](vite.config.js#L81-L97) | `npm test` reports 0 tests / 3 failed suites — Vitest 4.1.7 parallel-pool race. Tests pass individually. CI gate is currently broken. | Add `fileParallelism: false` to the `test` block. *(Originally proposed as `poolOptions: { forks: { singleFork: true } }` — refined to `fileParallelism: false` when Sprint 4 was executed; see Sprint 4 C-1 entry.)* **2026-06-17 update:** `pool: 'forks'` was added alongside `fileParallelism: false` — the race is in Vitest 4.1.7's default worker_threads pool under Node 24; switching pools sidesteps it, `fileParallelism: false` is kept as a second layer of safety. The race remains intermittent even with both settings (confirmed via repeated `npm test` runs during TS-08–12) — re-running is the workaround until upstream fixes the pool, not a sign of a real regression. |
...
- **2026-06-05** — ✅ **Full system i18n — Feature #13** (commit `f2a5bad`, branch `test`). Complete Arabic/English multi-language support with RTL layout. Stack: `react-i18next`, `src/locales/en.json` + `src/locales/ar.json` (~500+ keys), `src/lib/i18n.js` config. `AppearanceContext` manages language toggle; `dir="rtl"` applied to `<html>`. Every page, component, modal, toast, button, action, and label wrapped in `t()`. Key patterns documented: module-level functions use `import i18next from 'i18next'` singleton or receive `t` as parameter; portaled elements (NotificationBell panel) need explicit `dir` attribute since `createPortal` bypasses HTML inheritance.
