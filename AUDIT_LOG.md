...
|----|------|-------------|---------|-----|
| **C-1** | CI/Tests | [vite.config.js:81-90](vite.config.js#L81-L90) | `npm test` reports 0 tests / 3 failed suites — Vitest 4.1.7 parallel-pool race. Tests pass individually. CI gate is currently broken. | Add `fileParallelism: false` to the `test` block. *(Originally proposed as `poolOptions: { forks: { singleFork: true } }` — refined to `fileParallelism: false` when Sprint 4 was executed; see Sprint 4 C-1 entry.)* |
