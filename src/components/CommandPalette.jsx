  ...
  const search = useCallback(async (q) => {
    ...
    try {
      ...
      const [tickets, customers, products] = await Promise.all([
        db.supabase
          .from('rma_tickets')
          .select('id,rma_number,ticket_status,customer_name')
          .or(`rma_number.ilike.%${safe}%,customer_name.ilike.%${safe}%`)
          .limit(5),
        db.supabase
          .from('customers')
          .select('id,company_name,contact_person,email,mobile')
          .or(
            `company_name.ilike.%${safe}%,contact_person.ilike.%${safe}%,email.ilike.%${safe}%,mobile.ilike.%${safe}%`
          )
          .limit(5),
        db.supabase
          .from('products')
      ...
      const r = [
        ...
        ...(customers.data || []).map((c) => ({
          type: 'customer',
          id: c.id,
          primary: c.company_name || c.contact_person || '—',
          secondary: c.email || c.mobile || '',
          raw: c,
        })),
        ...(products.data || []).map((p) => ({
          type: 'product',
          ...
          secondary: `${p.brand?.brand_name || ''} ${p.sku ? '· ' + p.sku : ''}`.trim(),
          raw: p,
        })),
      ]
      setResults(r)
    ...
    <div className="fixed inset-0 z-[300] flex items-start justify-center pt-[15vh]">
      ...
      <div className="relative w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
          ...
          <kbd className="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 text-xs text-gray-500 bg-gray-100 rounded border border-gray-200 font-mono">
            Esc
          </kbd>
        </div>
        ...
        {results.length > 0 && (
          <ul ref={listRef} className="max-h-80 overflow-y-auto py-2">
            {results.map((item, idx) => (
              ...
              >
                <span
                  className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${TYPE_COLORS[item.type]}`}
                >
                  {ICONS[item.type]}
                </span>
                <div className="flex-1 min-w-0">
                  ...
                  {item.secondary && (
                    <span className="text-xs text-gray-500 truncate block">{item.secondary}</span>
                  )}
                </div>
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${TYPE_COLORS[item.type]}`}
                >
