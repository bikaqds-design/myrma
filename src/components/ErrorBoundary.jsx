...
/**
 * Translated with a defaultValue on every lookup. (Audit finding BUG-042.)
 *
 * This is the screen that renders when something has already gone wrong, and
 * a failed i18next initialisation is one of the things that can go wrong. A
 * bare `t('errorBoundary.title')` would then paint the raw key at the user, on
 * the one screen where legibility matters most. With a defaultValue the worst
 * case is English — which is exactly what this component showed before.
 *
 * A class component, so `useTranslation` is unavailable; the i18next singleton
 * is the same instance the hook wraps.
 */
const tr = (key, fallback) => i18next.t(`errorBoundary.${key}`, { defaultValue: fallback })
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-[#0b0f17] p-6">
        <div className="max-w-lg w-full bg-white dark:bg-[#121823] rounded-xl shadow-lg border border-red-200 dark:border-red-900 p-8">
          ...
          >
            ...
            {this.state.copied ? tr('copied', 'Copied!') : tr('copyDetails', 'Copy error details')}
          </button>
