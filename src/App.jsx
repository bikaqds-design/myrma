  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className={`fixed lg:static inset-y-0 left-0 z-30 ${sidebarCompact ? 'w-16' : 'w-64'} bg-gradient-to-b from-gray-900 to-gray-800 text-white flex flex-col transform transition-all duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        {!sidebarCompact && (
          <div className="p-4 border-b border-gray-700/60">
            <button
              ...
              className={`w-full bg-gray-700/50 rounded-xl p-3 space-y-3 text-left hover:bg-gray-700 transition-colors group ${currentPage === 'account' ? 'ring-1 ring-indigo-500' : ''}`}
            >
              <div className="flex items-center gap-3">
                {currentUser?.user_metadata?.avatar_url ? (
                  <img src={currentUser.user_metadata.avatar_url} alt="avatar" className="w-9 h-9 rounded-full object-cover flex-shrink-0 ring-2 ring-indigo-500/40" />
                ) : (
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0 uppercase">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="font-medium text-sm text-white truncate">{currentUser?.user_metadata?.display_name || currentUser?.email}</div>
                </div>
              ...
              <div className="flex items-center justify-between">
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 capitalize">{currentUserRole}</span>
                <button onClick={(e) => { e.stopPropagation(); handleLogout() }}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-gray-400 hover:text-white hover:bg-red-500/20 hover:border-red-500/40 border border-transparent transition-all duration-200">
        ...
        {sidebarCompact && (
          <div className="p-2 border-b border-gray-700/60 flex justify-center">
            <button onClick={() => handleNavigate('account')} title="Account"
              className={`w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white font-semibold text-sm uppercase ${currentPage === 'account' ? 'ring-2 ring-indigo-400' : ''}`}>
              {(currentUser?.user_metadata?.display_name || currentUser?.email || '?')[0]}
            </button>
          </div>
        )}

        {/* SEARCH SIDEBAR BUTTONS — hidden for now, CommandPalette still works via Ctrl+K / ⌘K
        {!sidebarCompact && (
          <div className="px-4 py-2 border-b border-gray-700/60">
            <button onClick={() => setCmdPaletteOpen(true)}
              className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-700/40 hover:bg-gray-700 text-gray-400 hover:text-white transition-colors text-xs">
              <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
              <span className="flex-1 text-left">Search…</span>
              <kbd className="font-mono text-gray-500">⌘K</kbd>
            </button>
          </div>
        )}
        {sidebarCompact && (
          <div className="px-2 py-2 border-b border-gray-700/60 flex justify-center">
            <button onClick={() => setCmdPaletteOpen(true)} title="Search (⌘K)"
              className="w-10 h-10 flex items-center justify-center rounded-lg text-gray-400 hover:text-white hover:bg-gray-700 transition-colors">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
        ...
        */}
        <nav className={`flex-1 ${sidebarCompact ? 'p-2' : 'p-4'} space-y-1 overflow-y-auto`}>
          ...
          <a href="/tracker" target="_blank" rel="noopener noreferrer" title={sidebarCompact ? 'Customer Tracker' : undefined}
            className={`w-full flex items-center ${sidebarCompact ? 'justify-center px-2 py-3' : 'gap-3 px-4 py-3'} rounded-lg transition-colors text-gray-300 hover:bg-gray-700`}>
            <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {(currentUserRole === 'admin' || currentUserRole === 'super_admin') && (
            <>
              ...
              <button onClick={() => handleNavigate('control-panel')} title={sidebarCompact ? 'Control Panel' : undefined}
                className={`w-full flex items-center ${sidebarCompact ? 'justify-center px-2 py-3' : 'gap-3 px-4 py-3'} rounded-lg transition-colors ${currentPage === 'control-panel' ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-700'}`}>
                <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Desktop notification bar — visible on all pages */}
        <div className="hidden lg:flex items-center justify-end px-6 py-2 bg-white border-b border-gray-100 flex-shrink-0">
          <div className="w-11">
            <NotificationBell
              notifications={notifications}
              currentUserEmail={currentUser?.email}
