            ...
            <div key={brand.id} className="border border-gray-200 rounded-lg">
              <div className="flex items-center justify-between p-4 hover:bg-gray-50">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  >
                    <svg className={'w-5 h-5 transition-transform ' + (expandedBrands[brand.id] ? 'rotate-90' : '')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
                  </button>

                  {brand.brand_logo_url ? (
                    <img src={brand.brand_logo_url} alt={brand.brand_name} className="w-10 h-10 object-contain rounded" />
                  ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-500">
                      {getCategoryCount(brand.id)} categories • {getBrandProductCount(brand.id)} products
                    </div>
                  </div>
                ...
                <div className="relative brand-menu">
                  <button 
                    onClick={(e) => {
                      e.stopPropagation()
                      setOpenBrandMenu(openBrandMenu === brand.id ? null : brand.id)
                    }}
              ...
              {expandedBrands[brand.id] && (
                <div className="border-t border-gray-200 bg-gray-50">
                  {getBrandCategories(brand.id).map((category) => (
                        ...
                        <div className="bg-white">
                          {getCategoryProducts(category.id).map((product) => (
                            <div 
                              key={product.id} 
                              onClick={() => handleViewProduct(product)}
                              className="flex items-center gap-3 p-3 pl-28 border-b border-gray-100 last:border-0 hover:bg-blue-50 cursor-pointer"
                            >
                              {product.product_image_url ? (
                                <img src={product.product_image_url} alt={product.product_name} className="w-8 h-8 object-cover rounded" />
                              ) : (
                              ...
                              <div className="flex-1">
                                <div className="text-sm font-medium text-gray-900 hover:text-indigo-600">{product.product_name}</div>
                          ...
                          {getCategoryProducts(category.id).length === 0 && (
                            <div className="p-3 pl-28 text-sm text-gray-500">No products in this category</div>
                          )}
                        </div>
                      )}
                    </div>
                  ))}
                  {getBrandCategories(brand.id).length === 0 && (
                    <div className="p-4 pl-16 text-sm text-gray-500">
                      No categories. Click the ⋯ menu above to add a category.
                    </div>
                  )}
                </div>
              )}
