...
export function TransferModal({ units: unitIds, warehouses, currentWarehouseId, onConfirm, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-modalNested flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="p-6 border-b border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900">{t('inventory.transferTitle')}</h3>
          <p className="text-sm text-gray-500 mt-1">
            {t('inventory.unitsSelected', { count: unitIds.length })}
          </p>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
          ...
          >
            {saving ? <Spinner size="sm" color="white" /> : t('inventory.confirmTransfer')}
          </button>
        </div>
      </div>
    </div>
