...
export function CompanyStockTab({
  ...
  const handleBulkTransfer = async (warehouseId) => {
    ...
    try {
      const moved = await db.inventory.transferUnits(selectedUnitIds, warehouseId, userEmail)
      ...
      else toast.success(t('inventory.unitsTransferred', { count: moved }))
      db.auditLog
        .log(
          userEmail,
                  ...
                  {paginated.map((g, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          {g.replacement > 0 ? (
                            <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-100 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300">
                              {g.replacement}
                            </span>
                          ) : (
                        ...
                        <td className="px-4 py-3">
                          {g.credit_note > 0 ? (
                            <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300">
                              {g.credit_note}
                            </span>
                          ) : (
                            <span className="text-gray-300 text-xs">—</span>
