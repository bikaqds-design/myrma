...
export const STATUS_META = {
  active_rma: { label: 'Active RMA', cls: 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800' },
  company_stock: { label: 'Company Stock', cls: 'bg-amber-100 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800' },
  sent_to_manufacturer: {
    label: 'Sent to Mfr',
    cls: 'bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800',
  },
        ...
        {pages > 1 && (
          <div className="flex items-center gap-1">
            ...
            {Array.from({ length: pages }, (_, i) => i + 1)
              ...
              .map((p, i) =>
                p === '…' ? (
                  <span key={`e${i}`} className="px-1 text-gray-500 text-xs">
                    …
                  </span>
                ) : (
                  <button
                    key={p}
                    onClick={() => onPage(p)}
                    className={`px-2.5 py-1.5 text-xs rounded-lg border transition-colors ${p === page ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                  >
                    {p}
                  </button>
                )
...
export function StatusBadge({ status }) {
  const m = STATUS_META[status] || {
    label: status,
    cls: 'bg-gray-100 dark:bg-[#1a2230] text-gray-500 dark:text-[#9aa4b2] border-gray-200 dark:border-[#212a38]',
  }
  return (
    <span
...
export function ResolutionBadge({ type }) {
  ...
  const m = RESOLUTION_META[type] || { label: type, cls: 'bg-gray-100 dark:bg-[#1a2230] text-gray-600 dark:text-[#9aa4b2]' }
  return (
    <span
...
export function BrandBar({ brands, groups, selected, onSelect }) {
  ...
  const cnt = (isActive, n) => (
    <span
      className={`text-[10px] px-1.5 py-0.5 rounded-full ${isActive ? 'bg-white/20 text-white' : 'bg-gray-100 dark:bg-[#1a2230] text-gray-500 dark:text-[#9aa4b2]'}`}
    >
      {n}
    </span>
  )
