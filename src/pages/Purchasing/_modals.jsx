...
function LineItemsEditor({ lines, setLines, products, t }) {
  ...
  const total = lines.reduce((sum, l) => {
    ...
    return sum + afterDisc + afterDisc * ((l.tax_pct || 0) / 100)
  }, 0)
  ...
  return (
    <div className="space-y-2">
      ...
      {lines.map((line, i) => (
        <div key={i} className="flex items-center gap-2 flex-wrap">
          ...
          <Input
            ...
            className="w-20" placeholder={t('purchasing.qty')}
          />
          <Input
            type="number" min="0" step="0.01" value={line.unit_cost}
            onChange={(e) => updateLine(i, { unit_cost: parseFloat(e.target.value) || 0 })}
            className="w-24" placeholder={t('purchasing.unitCost')}
          />
          <Input
            type="number" min="0" max="100" value={line.discount_pct || 0}
            onChange={(e) => updateLine(i, { discount_pct: parseFloat(e.target.value) || 0 })}
            className="w-16" placeholder={t('salesDocuments.fDiscount')}
          />
          <Input
            type="number" min="0" max="100" value={line.tax_pct || 0}
            onChange={(e) => updateLine(i, { tax_pct: parseFloat(e.target.value) || 0 })}
            className="w-16" placeholder={t('salesDocuments.fTax')}
          />
          <button onClick={() => removeLine(i)} className="text-red-500 hover:text-red-700 text-sm px-1">✕</button>
        </div>
      ))}
      ...
      <div className="text-right text-sm font-semibold text-[#211f1b] dark:text-[#e8ebf0]">
        {t('purchasing.total')}: {total.toLocaleString(undefined, { maximumFractionDigits: 2 })}
      </div>
    </div>
  )
}
...
export function CreatePurchaseOrderModal({ mode = 'create', initial, onClose, vendors, userEmail, onSuccess }) {
  ...
  return (
    <Modal open onClose={onClose} title={t(mode === 'edit' ? 'salesDocuments.edit' : 'purchasing.newPurchaseOrder')} className="max-w-3xl">
    ...
    <ModalOverlay onClose={onClose}>
      <ModalCard aria-label={t('purchasing.recordPayment')} className="dark:bg-[#121823] border border-[#e6e9ef] dark:border-[#212a38] max-w-2xl w-full">
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 sm:px-6 py-4 border-b border-[#e6e9ef] dark:border-[#212a38] bg-white dark:bg-[#121823] rounded-t-2xl">
          <h2 className="text-lg font-bold text-[#211f1b] dark:text-[#e8ebf0]">{t('purchasing.recordPayment')}</h2>
          <button onClick={onClose} aria-label={t('common.close')} className="text-[#6c6760] dark:text-[#9aa4b2] hover:text-[#211f1b] dark:hover:text-[#e8ebf0]">
