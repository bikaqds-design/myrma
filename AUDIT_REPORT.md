...
**Tracked elsewhere, not audit bugs:** the UI/UX backlog in `design/BACKLOG.md` (0 Critical; the remaining High items are long-running consistency work — hard-coded colours, empty states, component-kit adoption — and it records that none blocks launch); Warehouse Module R1's deferred R2 items (see `CLAUDE.md`) — its manual QA passed on 2026-08-05/06 and again as a regression re-run on 2026-09-17, which raised four UI/reporting findings recorded in `docs/archive/WAREHOUSE_R1_TEST_CHECKLIST.md`. All are closed: delivered units counted in Main and Physical Total (`20260874`), the stale Branches drawer and Escape discarding the Create Ticket form (UI fixes), and one withdrawn as not a defect.
...
* ~~**Still open, and deliberately not done here:** enforcing `supabase db diff` in CI~~ **Ledger check in CI — ADDED 2026-09-16** (`20260868_applied_migration_versions.sql`, `scripts/migration-drift.mjs`, `.github/workflows/migration-drift.yml`).
* **It drifted a third time before this landed**, which is the case for the check in one line. Every migration applied through the dashboard/MCP `apply_migration` is recorded under a generated timestamp (`20260916084108`) rather than its file's version, so by 2026-09-16 forty-three rows (`20260825`–`20260867`) no longer matched their files and `supabase db push` would have re-run every one of them. Owner-approved, all 43 were pointed back at their file versions in a single all-or-nothing block (each file had to match exactly one row). Result: **199 ledger rows for 199 files**; the seven remaining 14-digit versions are this finding's own intentional renames (`20260524000001` and siblings).
* **What the check does.** On every push to `main`, daily, and on demand, it compares production's applied versions with the file prefixes in `supabase/migrations/` and fails on each of the three shapes this finding has actually taken: a **file with no row** (not applied, or recorded under a generated version — the 2026-09-16 drift), a **row with no file** (the 2026-09-07 drift, where three fixes existed only in production), and **two files sharing a version** (the original 2026-09-06 cause, since `version` is the primary key). It does not run on pull requests, where a new migration is supposed to be ahead of production.
* **How it reads production — owner's choice of no database credential in CI.** `rma_applied_migration_versions()` returns the applied version numbers as a single `text[]` (one value, so the 1 000-row Data API cap can never truncate it) and is executable by `anon`, using the URL and anon key the integration tier already holds; it is a deliberate, named exception to `20260841`'s revoke-anon default, and its migration refuses to finish unless the grant took. It discloses version numbers only — the same prefixes as the public file names — and `anon` still cannot read `supabase_migrations` directly (verified: "permission denied for schema supabase_migrations"). Without the secrets (a fork) the script skips with a visible `::warning::` annotation; **with them, failing to read production fails the run**, so the check cannot switch itself off by breaking.
* **Verified before merge.** Dry run on production (rolled back): as `anon`, 199 versions returned. The comparison logic run against production's real versions and this branch's 200 files reported exactly one problem — `20260868` itself, not yet applied — and nothing else. Tests: `src/test/migrationDrift.test.js` (13, including the exact generated-timestamp case seen live and a check that the repository has no duplicate or badly named migration files today).
* **Scope, stated plainly:** this compares the **ledger** with the **files**. It is not `supabase db diff`: a change made in the SQL editor *without* going through a migration leaves no ledger row and no file, and this check cannot see it. Diffing the schema itself still needs the CLI and a database connection in CI, which the owner chose not to put there. **Operating rule that follows:** after every `apply_migration`, point its ledger row at the file's version, or the next push to `main` goes red.
...
* **Status: FIXED 2026-09-14.** Public sign-up was turned off by the owner, so the mechanism is closed as well as the ten grants it exposed (history below).
* **Verified from outside the dashboard, not taken from the toggle:** `/auth/v1/settings` returns `disable_signup: true`, and the Auth service log shows the configuration reload at the moment of the save (09:35 UTC). The first attempts that day did not save; the log showed no reload for them, which is how that was caught rather than recorded as done.
* **Checked before switching it off that nothing legitimate depends on public sign-up:** the app never calls `auth.signUp` (removed under BUG-062); staff accounts are created by `admin-invite-user` (`auth.admin.inviteUserByEmail`) and `admin-reset-password` (`auth.admin.createUser`), both admin APIs that the setting does not affect; no external identity providers are enabled. Not exercised end to end — that would mean sending a real invitation — so the next invitation is the confirmation.
* **History, 2026-09-13 (PARTLY FIXED):** the waiting grants were closed; the mechanism that made them dangerous was an owner action.
...
* ~~**Not fixed, and still open:** the endpoint still accepts any ticket UUID without proof the caller knows the RMA number, and the rate limiter is still per-instance rather than backed by a table.~~ Both now closed:
* **Rate limiter — closed 2026-09-13 under BUG-067.** `20260844_public_track_rate_limit.sql` added `rma_public_track_hit`, a database-backed per-caller count (salted SHA-256 of the address) shared by every instance and surviving cold starts; the in-memory bucket only sits in front of it.
* **Proof of the RMA number — FIXED 2026-09-15; `public-track` deployed (v19) and verified against production 2026-09-15.** Live: a `comments` call with no number is refused 400, with another ticket's number 404, with a wildcard (`RMA-21052026%`) 404, and with the right number 200 returning only the public columns; `addComment` refuses a wrong or missing number the same way, and `ticket_comments` was unchanged at 25 rows afterwards. `comments` and `addComment` now require `rmaNumber` alongside `ticketId`, and `provenTicketId()` (`supabase/functions/_shared/trackerProof.ts`) accepts the request only when that number names a ticket and it is the ticket asked for; an unknown number and a mismatched id both get the same `404 Ticket not found`, so the endpoint cannot be used to test ids against numbers. The comment is stored against the proven id, reply threading is checked against it, and an insert now returns only the public comment columns (it returned the whole row, including `user_email`). Reading a ticket's thread by id alone was the same gap and is closed with it. The tracker page and `rmaTracker` send the number (`src/api/db/tickets.ts`, `src/pages/RMATracker.jsx`). Tests: `src/test/trackerProof.test.js` (13 — the proof rule including case, trimming, mismatch, unknown, missing/over-long input and a failing lookup; LIKE escaping; the function's use of the proof in both actions; the client sending the number; the function parses).
...
* **Deleting a ticket now returns its parts (owner decision, 2026-09-17, `20260873_ticket_parts_return_stock_on_delete.sql`).** Before, `ticket_parts` cascaded away with the ticket and the stock stayed consumed. An AFTER DELETE trigger on `ticket_parts` now returns each row's quantity to its part however the row goes — `rma_ticket_part_remove` or the cascade — so `rma_ticket_part_remove` no longer restocks itself. The trigger writes `parts` directly, so a delete made without a user session (SQL editor, the planned data reset) is not blocked by the staff check. Rolled-back probe on production, 8/8: removal restocks once (not twice); a technician cannot delete a ticket; an admin deleting a ticket gets its remaining parts back; a session-less delete restocks; other tickets are untouched; direct `ticket_parts` deletes stay refused; security invariants unchanged. `ticket_parts_atomic.sql` gained the ticket-delete case.
...
* ~~**The ledger half remains open and is not claimed as fixed.**~~ **Ledger half FIXED 2026-09-16** (`20260866_transfer_units_ledger.sql`). Every remaining path now writes `stock_moves`, so the Warehouse Dashboard and margin reporting can see a unit move or leave.
* **The R2 note said to route the screens through `transfer_stock`; that was re-checked rather than followed.** The comment on `transferUnits` warned *“the feature sets differ; don't merge blindly”*, naming the System Pool / null-warehouse destination. That case is gone: `TransferModal` dropped it in the Warehouse R1 redesign, and in production all 441 units have both a product and a warehouse, so there is no unplaced pool left to preserve. What genuinely differs is that `transfer_stock` moves **one** unit per call and needs the product and source warehouse passed in, while the three screens transfer a whole selection — up to everything in a warehouse — and know only the unit ids. Looping it from the browser would leave a transfer half-done when one unit in the middle is refused. So the fix is a set-based sibling, `transfer_units(uuid[], uuid, text)`, on the same ledger conventions (`move_type 'transfer'`, `ref_type 'unit'`, both warehouse ids in `from_status`/`to_status`): one transaction, one row per unit that actually moves, a unit already at the destination a no-op rather than an error.
* **The 20260831 guard does not protect this path, so the function repeats its rules.** That guard is `SECURITY INVOKER` and returns early unless `current_user` is `authenticated`/`anon` — which is exactly how the stock RPCs are allowed to move units into system locations legitimately. Anything reached through `SECURITY DEFINER` therefore carries no protection from it. `transfer_units` refuses a **reserved** unit and a **system or archived** destination itself, and `supabase/tests/transfer_units_ledger.sql` asserts both directly rather than inheriting the guard's own result.
* **Who may transfer was settled, not assumed:** `rma_is_manager_or_above()`, matching `transfer_stock`. Not a narrowing in practice — `canDo('inventory','transfer')` is `false` in every role default and no stored permission row in production grants it, so the button is visible only to admin/super_admin, who bypass `canDo`. Owner's decision 2026-09-16.
* **`mark_batch_resolved` still closes every unit, and that is now a decision rather than an inheritance.** Put to the owner on 2026-09-16 with the alternative (a "Replacement Rcvd" batch returning its units to company stock, which an unused client dialog still describes): declined — the unit sent to the manufacturer does not come back, and a replacement arriving is a separate receipt against a vendor invoice. Both `mark_batch_sent` and `mark_batch_resolved` now write one ledger row per unit under a new `manufacturer_batch` doc type, so the closure is recorded even though the semantics are unchanged.
* **`resolveUnits` was deleted rather than migrated.** It set `status`/`resolution_type` straight on the row and had **no caller anywhere in `src/`** — the same dead state `resolve_units()` was found in during the R1 redesign. Its unreferenced dialog (`ResolveModal` in `ManufacturerTab.jsx`, whose text still promises "moves to Company Stock") is left in place and noted here; removing dead UI is not part of this fix.
* **Verifying this migration is what exposed BUG-087** — the `IF NOT rma_is_*()` guard idiom fails open for a caller with no current role. `transfer_units` and both batch RPCs use `COALESCE(…, false)`; the ~34 other functions carrying the bare idiom are filed above and not fixed here.
* Tests: `supabase/tests/transfer_units_ledger.sql` (11 checks — a reference script, **not run by CI**: the `db-tests` job has been `if: false` since 2026-08-07, so the step added for it never executes; the checks were run against production in a rolled-back transaction with live-active accounts, 11/11) and `src/test/transferLedger.test.js` (11 — no direct `warehouse_id` write survives, each screen passes the signed-in user and reports what actually moved, and the function keeps the guard's rules, its JWT actor and its EXECUTE lockdown).
...
* Description: when the caller has **no current role row** — suspended, access expired, or an auth account with no `user_roles` row at all — `rma_user_role()` returns NULL rather than a role. `NULL IN ('super_admin','admin','manager')` is NULL, `NOT NULL` is NULL, and **a plpgsql `IF` on NULL does not fire**, so the guard raises nothing and execution falls through into the body. The identical helper is safe in an RLS policy, because a policy treats NULL as "no" — it fails closed. A plpgsql `IF` fails **open**. That asymmetry is what makes this easy to miss: the same expression protects the tables correctly and the RPCs not at all.
* How to reproduce: in a rolled-back transaction, `set_config('request.jwt.claims', '{"email":"nobody-with-no-role@example.com","role":"authenticated"}', true)` then `SELECT count(*) FROM public.rma_staff_directory()`.
* Actual (measured live 2026-09-16): `rma_user_role()` → NULL, all three helpers → NULL, and `rma_staff_directory()` — manager-and-above only — **returned all 17 staff rows**.
* Expected: a caller without a current role is refused by every one of these functions.
* Impact: the state is not hypothetical. Production has **10 suspended `user_roles` rows** (the `@test.com` seed cohort, including one holding `super_admin`) and at least one real auth account with no role row at all (`ahmed@qdsegypt.com`, which the UI correctly shows an access-denied screen). Suspension is recorded in `user_roles`; it does not by itself invalidate an existing Supabase session, and these RPCs are granted to `authenticated`. So a suspended staff member with a live token — the exact case `rma_revoke_user_sessions` exists to handle — keeps the ability to post and void invoices, issue credit notes, record vendor payments, move and adjust stock, and cascade-delete customers. RLS still protects direct table reads and writes, which is why nothing about this is visible in the UI.
* Suggested fix: make the three helpers return false instead of NULL (`SELECT COALESCE(rma_user_role() IN (…), false)`) — one change that closes every caller at once, and a no-op for the policies, which already treat NULL as false. Belt and braces: `COALESCE(…, false)` in the guards themselves.
* Confidence: **Confirmed by execution against production** (read-only probe, rolled back).
* ~~**Status: FIXED for `20260866`'s own three functions only**~~ **Status: FIXED 2026-09-16 for every function** (`20260867_role_guards_fail_closed.sql`, owner's choice of the helper-level fix). Two parts:
  1. **The four helpers return false instead of NULL** — `rma_is_staff`, `rma_is_admin`, `rma_is_manager_or_above`, and `rma_can_handle_cash`, which had to be included separately because `false OR rma_user_role() = 'accountant'` is still NULL. That closes every `IF NOT helper() THEN` guard (~34 functions) in one place.
  2. **Seven guards that compare the role directly are wrapped in `COALESCE(…, false)`**, because the helper fix cannot reach `false OR rma_user_role() = 'sales_rep'`: `cancel_sales_order`, `convert_quotation_to_so` and `crm_convert_lead` (still open without this), plus `adjust_part_quantity`, `create_manufacturer_batch`, `link_serial_to_rma_ticket` and `move_rma_units` (closed by part 1 already, wrapped so no guard depends on a helper's NULL handling). They are rewritten from their **live** definitions with only the guard text changed — not re-declared from older migration copies, which is how a fixed bug comes back — and the migration refuses if any function has an unexpected number of guards.
* **Why changing shared helpers was safe, checked rather than assumed.** NULL → false can only change an outcome where a helper is negated, compared or null-tested for a positive purpose. No RLS policy does that — every one of the ~50 policies that use the helpers uses them positively, where NULL and false both mean "no" — and the only positive function uses are `IF public.rma_is_admin() THEN <allow>` in three trigger guards, where both skip the branch. The migration re-checks the policy half at apply time and refuses if a policy has started negating a helper. `rma_user_role()` itself is deliberately untouched: a sentinel such as `''` would turn every `rma_user_role() <> 'viewer'` policy TRUE for a role-less caller.
* **Dry-run against production (rolled back) before applying:** every precondition and rewrite matched; a role-less caller was refused by `rma_staff_directory`, `cancel_sales_order`, `convert_quotation_to_so`, `transfer_stock` and `move_rma_units`, all with "Not authorized"; and every real role kept exactly what it had — super_admin all true (directory still 17 rows), manager staff/manager/cash true and admin false, technician and sales_rep staff only, and the suspended seeded manager false throughout.
* Tests: `supabase/tests/role_guards_fail_closed.sql` (7 checks, a reference script **not run by CI** — the `db-tests` job is `if: false`; verified instead by the rolled-back dry run and live probes on production described above — role-less, suspended and expired callers refused across all three guard shapes; every real role unchanged; no unwrapped direct role guard anywhere; no policy negating a helper) and `src/test/roleGuardsFailClosed.test.js` (9 — reads the whole migration history, so a later migration re-declaring a helper without COALESCE, or adding a bare direct role guard, fails CI).
* **Regression check in CI (2026-09-16):** `20260869_security_invariant_counts.sql` exposes, to anon, counts only — unwrapped role guards, helpers without COALESCE, policies negating a helper, anon-executable functions — and `tests/integration/security-invariants.test.ts` asserts the first three are 0 on **production** on every CI run (the integration tier, which really runs since the Node 22 fix in #28). Dry-run verified, including a sensitivity probe: re-declaring `rma_is_admin` without COALESCE inside the rolled-back transaction moved `helpers_without_coalesce` from 0 to 1. `rpc-auth.test.ts` also now checks that anon is refused by `convert_quotation_to_so`, `transfer_units`, `move_rma_units`, `mark_batch_sent` and `rma_staff_directory`.
* **Note for whoever re-tests this:** both seeded test accounts used by the SQL test files (`karim.nasser@test.com`, `hana.sayed@test.com`) are **suspended on production**, so on live they exercise the role-less path rather than manager/technician. In a freshly seeded local database they are active. **Correction (2026-09-16):** an earlier version of this note said CI passed this file because the accounts are active there — wrong: the CI `db-tests` job is `if: false` and never ran it. Production caught the defect because the file was run against production, and only there. Use a live-active account (e.g. the `manager` and `technician` rows in `user_roles` with `status='active'`) when running these against production.
...
* Confidence: Confirmed by code reading
* **Status: FIXED 2026-09-14** (pull request #2, merged). The owner decided the open question below: import customers the add-customer form would refuse, and **name each one that is missing a contact person or a mobile**, the same way damaged phone numbers are named. RMA intake finds a customer by phone, so a customer with no mobile is one the counter cannot look up until someone adds it.
* The rule is `missingCustomerFields()` in `src/lib/importValidation.js`, with four tests. Both reports (missing details and damaged numbers) now run **only after the insert succeeds**; the phone warning previously ran before it and said "imported" even when the insert then failed.
* **History, 2026-09-07 (PARTLY FIXED):** two of the three problems were closed; the third was a deliberate non-change, explained below.
...
* Still unchanged, and not part of what this finding needed: the three hand-rolled `parseCSVLine` implementations, and duplicate detection still comparing against the ≤5,000 rows the page has loaded.
...
* **Residual closed 2026-09-16 — ending sessions is now enforced by the database, for every way access is lost** (`20260870_revoke_sessions_on_access_loss.sql`). The 2026-09-07 fix revoked from the browser, after the status write, best-effort, and only for `suspended`/`locked`. That left five holes: **`deactivated` never ended a session**; a status change made anywhere but that one screen (SQL editor, restore, a future bulk action, or the browser call failing — swallowed to Sentry by design) ended nothing; an expiry set in the past ended nothing; deleting a user's role row ended nothing; and an expiry that simply *passes* ended nothing, because no row changes at that moment. Each left a refresh token minting new access tokens indefinitely (the database refused them all since BUG-087, but the session should not exist).
  * An `AFTER UPDATE OF status, access_expires_at, user_email OR DELETE` trigger on `user_roles` ends the user's `auth.sessions` (refresh tokens cascade — checked at apply time) **in the same transaction**, so a suspension can no longer succeed while the revoke fails. It fires for `suspended`, `locked`, `deactivated`, an expiry at or before now, an email change (for the old address) and deletion. **Never for `pending`**: an invitee signs in while pending and then calls `rma_accept_invitation`.
  * A pg_cron sweep every 15 minutes (`end-sessions-without-access`) ends sessions for anyone with a role row whose access is no longer current and who is not pending — the passing-expiry case, and a backstop for anything else. Logins with no role row are deliberately left alone (that is the gap between signing up and the role row existing), and already get nothing from the database.
  * The browser-side revoke in `users.updateUserStatus()` is removed; the admin-facing `rma_revoke_user_sessions*` functions and the Account Settings session list are unchanged. The new functions have no authorization check of their own and are therefore not executable by `anon` or `authenticated` (the migration refuses to finish otherwise).
  * **Verified by a rolled-back behavioural probe on production** with throwaway logins: suspended → 0 sessions and 0 refresh tokens; reactivated + new login → 1; deactivated → 0; pending (including a sweep) → 1, kept; expiry set in the past → 0; expiry passing silently → 1 until the sweep removed it → 0; role row deleted → 0; an unrelated user's session untouched throughout. Production at the time: no suspended, locked or deactivated account held a login at all, and the one login without a role row had no sessions — so nothing was live to revoke; this closes the path, not a current exposure.
  * **Residual, stated plainly:** ending a session stops refresh, not an access token already issued, which stays valid until it expires (Supabase default: 1 hour). Within that window the database refuses it (BUG-087) and Edge Functions check status (BUG-021).
...
* Confidence: Confirmed by execution
* **Status: FIXED 2026-09-14** (pull request #1, merged). `typescript` 7.0.2 is a dev dependency, `npm run typecheck` exists, and CI runs a **Type check (zero errors)** step between lint and build. The first CI run on the pull request passed that step, along with the unit tests, lint and the production build.
* **The count was 96, not 83,** once `tsc` ran against the whole tree. 48 of them came from `.ts` files reading types out of the project's own JavaScript (`client.js`, `sentry.js`, `dates.js`); `allowJs` with `checkJs` off lets them, and typing the client resolved most of the implicit-any parameters too. The rest were real annotation fixes: selects built from runtime column lists (cast to their declared row types, since the typed select parser cannot read them), `documents.ts` `applyFilters` typed as the builder before `.select()`, and three small ones in `inventory.ts`, `rmaStageMoves.ts` and `stableEmpty.ts`.
* **One error was left in place on purpose and is labelled.** `WhatsAppProvider.ts` imports `../../api/client.js`, which resolves to a file that does not exist, so a send through that provider fails at runtime. It carries `@ts-expect-error` naming the problem and BUG-006; it is not fixed because WhatsApp work is on hold. Correcting the path will make `tsc` reject the now-unused directive, so the fix cannot be forgotten.
* **History:** **Not attempted 2026-09-13, and the reason is a blocker rather than a choice.** Closing this means adding `typescript`, fixing the 83 errors and gating CI on it — and the errors cannot be fixed without running `tsc`, which means installing the dependency first. This machine currently has about half a gigabyte of memory free; `npm install` and the production build both fail on it today. Adding the dependency and the CI gate *without* fixing the errors would turn CI red on HEAD, which is the state BUG-016 was raised to get out of. It needs a machine that can run the compiler.
...
* ~~**Still open, deliberately:** nothing covers the authenticated-role bypasses behind BUG-001/002/010/011.~~ **Covered 2026-09-16 — without a second project and without any login** (`supabase/tests/authenticated_role_probes.sql`). The blocker was stated as "signed-in fixtures writing to a database". Neither half was needed. A signed-in API call is, to the database, a set of JWT claims plus the `authenticated` role — so the script sets exactly the claims PostgREST sets (`email`, `role`, `sub` from `auth.users`) and switches to `authenticated`, and row-level security, table grants and guard triggers apply as they do for a browser. It runs inside one transaction that ends in a forced rollback, so nothing it writes survives, and the data is disposable test data in any case.
* **Owner's choice of scope:** a reference script run by hand, not CI. CI holds only the anon key; running this from CI would need a database credential or per-role test passwords in GitHub, and a reopened loophole would then write a real row. `db-tests` stays disabled.
* **What it checks, one active account per role (picked at run time, so it survives a data reset; a role with no account is SKIPPED, never passed):** BUG-001 — payments refused for viewer, sales rep and manager (the table has no client INSERT grant at all), vendor payments refused for viewer and manager; BUG-002 — a posted invoice cannot be rewritten by its own rep or by a manager; BUG-034 — a rep cannot post their own draft by hand **but can still cancel it**; BUG-004 — a rep cannot mark their own order delivered **but can still send it**; BUG-010 — a technician cannot set `reservation_status` or `unit_cost_base` **but can still edit notes**, and a viewer cannot; BUG-011 — viewer and sales rep cannot add ticket resolutions **but a technician can**, a viewer cannot rewrite notifications, change company appearance or queue WhatsApp jobs, and a forged notification author is replaced by the signed-in email; BUG-087 — a technician is refused the manager-only `transfer_stock`.
* **Only the right refusal counts.** A refusal passes on 42501 (RLS or grant), P0001 or 23514 (a guard) or an UPDATE that RLS made invisible — never on some other error, since a NOT NULL or foreign-key failure would mean the probe is broken, not that the database said no. The first run caught exactly that kind of blind spot in the probe itself: the inventory guard refuses with 23514, which the first matcher did not accept, so two correct refusals read as failures until the matcher was widened. Pairing every refusal with a legitimate action beside it is deliberate — a database that refused everything would otherwise pass.
* **Run against production 2026-09-16, the committed file exactly as written: 24 checks, 24 passed, 0 skipped.**
...
* **Status: FIXED 2026-09-14, within what the Free plan allows.** Public sign-up is off and verified (see BUG-084). The one remaining recommendation is blocked by the plan, not outstanding work:
* **Leaked-password protection (HIBP) needs Supabase Pro.** The project is on the Free plan; the owner confirmed on 2026-09-14 the setting is unavailable there. The security advisor will keep reporting `auth_leaked_password_protection` until the plan changes — that warning is expected, not a regression. Turning it on is part of any upgrade: Authentication → Sign In / Providers → Email → Prevent use of leaked passwords (it moved there from Attack Protection).
* **History, 2026-09-13 (PARTLY FIXED):** everything readable from outside the dashboard was read and recorded here; the remaining changes were owner actions.
* **Read from production 2026-09-13:** `disable_signup = false` (public sign-up was **on** — now off, 2026-09-14), `mailer_autoconfirm = false` (sign-up requires confirming the email), email provider enabled — all from `/auth/v1/settings`. Leaked-password protection is **disabled**, per the security advisor. `ALLOWED_ORIGINS` is set (see BUG-056).
