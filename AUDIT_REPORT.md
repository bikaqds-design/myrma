...
* **Status: FIXED for `20260866`'s own three functions only** (`transfer_units`, `mark_batch_sent`, `mark_batch_resolved` now `COALESCE(…, false)`), applied and re-verified live: a role-less caller is refused, an active manager still succeeds, an active technician is refused. `supabase/tests/transfer_units_ledger.sql` CHECK 7b and `src/test/transferLedger.test.js` pin it. **The other ~34 functions are NOT fixed** — changing shared helpers used by every RLS policy in the database is an owner decision, not something to slip into a migration about warehouse transfers.
